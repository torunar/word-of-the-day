(function (eConfigForm, eCard, eIcon, eWord, eIconField, eWordField, eCardImage, eDownload, eIconSelectors) {
  ...
  /** @var {HTMLButtonElement[]} eIconSelectors **/
  ...
  function render(icon, word) {
    ...
    let eActiveIconSelector = eIconSelectors.filter(function (eIconSelector) {
      // remove active class
      eIconSelector.className = eIconSelector.className.replace(eIconSelector.dataset.activeClass, '').trim()

      return eIconSelector.dataset.icon === icon
    })[0]
    eActiveIconSelector.className += ' ' + eActiveIconSelector.dataset.activeClass
  }
  ...
  function addIconSelectorHandler() {
    eIconSelectors.map(function (eIconSelector) {
      eIconSelector.onclick = function (event) {
        event.preventDefault()

        eIconField.value = eIconSelector.dataset.icon
  ...
  document.getElementById('render_image'),
  document.getElementById('download'),
  [...document.getElementsByClassName('icons__selector')]
)
